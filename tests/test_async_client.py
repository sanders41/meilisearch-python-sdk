...
@pytest.fixture
async def remove_default_search_key(default_search_key, async_client):
    await async_client.delete_key(default_search_key.key)
    yield
    key = KeyCreate(
    ...
    await async_client.create_key(key)
...
async def test_create_snapshot(async_test_client, async_index_with_documents):
    index = await async_index_with_documents()
    response = await async_test_client.create_snapshot()
